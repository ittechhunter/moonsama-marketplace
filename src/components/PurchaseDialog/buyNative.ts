...
export const buyNative = (ppu?: BigNumber, total?: BigNumber, inputAmountText?: string, giveAssetDecimals=0, getAssetDecimals=0) => {
    ...
    // input amount to buy is in ether
      ...
      userGet = inputAmount;
      sendAmount = userGet;
      userGive = ppu?.mul(sendAmount).div(parseUnits('1', getAssetDecimals)) ?? BigNumber.from('0');

      //userGive = sendAmount
      userGetDisplay = Fraction.from(userGet?.toString(), getAssetDecimals)?.toFixed(getAssetDecimals > 0 ? 5: 0);
      userGiveDisplay = Fraction.from(userGive?.toString(), giveAssetDecimals)?.toFixed(giveAssetDecimals > 0 ? 5: 0);
